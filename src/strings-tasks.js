...
function removeFirstOccurrences(str, value) {
  if (str.indexOf(value) !== -1) {
    ...
    arrStr.splice(str.indexOf(value), value.length);
    return arrStr.join('');
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) !== -1) {
    const arrStr = str.split('');
    arrStr.splice(str.lastIndexOf(value), value.length);
    return arrStr.join('');
  }
  return str;
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((char) => {
      ...
      if (letterCode <= 77 && letterCode >= 65) {
        return String.fromCharCode(letterCode + 13);
      }
      if (char === '?' || char === ' ' || char === '!') {
        return char;
...
function getCardId(value) {
  ...
  return deck.findIndex((val) => val === value);
}
